 ...
 * Read a config file
 *
 * @param {string} file  Filename of JSON config file
 * @param {object} config  Existing global config
 *
...
function readConfig(file, config) {
  ...
  var option_keys = ['hide_before_and_after', 'output_dir', 'before_dir', 'after_dir', 'threshold', 'filter'];
 ...
 * help
 *
 * @param {string} msg  Error message so show prior to help
 */
...
/**
 * Convert config object to commandline options for phantomjs
 *
 * @param {object} config
 *
 * @return {array}
...
exports.getProcessingOptions = function (config) {
  var option_keys = ['hide_before_and_after', 'output_dir', 'before_dir', 'after_dir', 'threshold', 'verbose', 'filter'];
  var i, results = [];

  for (i = 0; i < option_keys.length; i++) {
    if (typeof config[option_keys[i]] === 'undefined' || config[option_keys[i]] == false) {
      continue;
...
/**
 * Process incoming command line options
 *
 * @param {array} argv  Array of args
 *
 * @return {object}
 */
exports.processArgs = function (argv) {
  ...
  for (i = 0; i < argv.length; i++) {
    ...
    if (kv === null) { // Set mode
      ...
      } else if (null !== argv[i].match(/(http|https|file):\/\/.+/)) {
        if (null !== argv[i+1].match(/(http|https|file):\/\/.+/)) {
          config.mode = 'compare';
          config.url = argv[i];
          config.raw_options.push('--url2=' + argv[i+1]);
          i += 1;
        } else {
          return {
            "mode": 'help',
            "message": 'Got one URL but then not another one immediately: ' + argv[i+1]
          };
        }
