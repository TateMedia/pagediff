...
function readConfig(file, config) {
  ...
  for (item in data) {
    if (['size', 'batches', 'hide_before_and_after'].indexOf(item) !== -1) {
      if (item === 'hide_before_and_after') {
        if (data.hide_before_and_after === 'true' || data.hide_before_and_after === 'True' || data.hide_before_and_after === '1') {
          config.hide_before_and_after = true;
        } else {
          config.hide_before_and_after = false;
        }
      } else {
...
exports.processArgs = function (argv) {
  var config = {'mode': 'invalid', 'size': [1024, 768], 'hide_before_and_after': false};
